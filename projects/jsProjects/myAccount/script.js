    ...
    if (description != "" && amount != "") {
        manager.addAction(
            new Action(
                ...
                amount,
                currentDate,
                manager.savedAmount || 0,
                manager.totalSavings
            )
        );
        manager.calcBalance();
...
function showActions() {
    ...
    <div class="row mb-4">
            <h1 class="text-center display-1 text-success">החשבון <i class="text-danger fa-solid fa-chart-pie"></i> שלי</h1>
        </div>
        ...
        <div class="row">
            <h6 class="section-title text-primary display-6 p-2">חיסכון</h6>
            <div class="row mb-4">
                <div class="col-sm-4 card card-custom p-4 bg-light">
                    <h5 class="text-success h1">
                        <span class="text-primary display-6 my-3">סה"כ</span>
                        <hr>
...
window.getAll = function getAll() {
    ...
    for (let action of storedActions) {
        ...
        tbody.innerHTML += `
        <tr>
            <td class='fw-bold' style="color: ${colors};">${action.description}</td>
            <td class='fw-bold' style="color: ${colors};">${action.amount} <i class="fa-solid fs-6 fa-shekel-sign"></i></td>
            <td class='fw-bold' style="color: ${colors};">${action.date}</td>
            <td>
